...
def ensure_unlocked() -> None:
    ...
    st.write("Subscribe to unlock the app — or, if you already have a subscription, unlock with your Subscription ID.")
    ...
    with left:
        ...
        st.caption("Enter your email (for receipt + access records), then subscribe.")

        subscribe_email = st.text_input(
            "Email (for receipt + access records)",
            key="subscribe_email_v1",
            placeholder="your@email.com",
        )

        # Only show the PayPal embed AFTER the user has entered an email.
        # This keeps the PayPal iframe/script away from the manual unlock inputs.
        if not subscribe_email.strip():
    ...
    with right:
        ...
        st.caption("Use this if you already have a PayPal Subscription ID.")

        # Put the manual unlock in a form to prevent reruns on every keystroke.
        with st.form("manual_unlock_form", clear_on_submit=False):
            manual_email = st.text_input(
                "Email (required for unlock)",
                key="manual_email_v1",
                placeholder="your@email.com",
            )
